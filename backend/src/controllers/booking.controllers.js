...
const HostelBookings = require('../models/hostelbooking.model');
...
exports.placedHostelBookingOrder = async (req, res) => {
  try {
    // finding by room id
    let myRoom = null;

    if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
      myRoom = await Hostels.findById(req.params.id);
    } else {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
    ...
    // check room status is`unavailable`
    if (myRoom.hostel_status === 'unavailable') {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
        'Sorry! Current your sleeted room can\'t available'
    ...
    // check room status is `booked`
    if (myRoom.hostel_status === 'booked') {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
        'Sorry! Current your sleeted already booked. Please try again later'
    ...
    // save room data in database
    const booking = await HostelBookings.create(data);
  ...
  try {
    ...
    const mapperBooking = findBooking?.map((data) => ({
      ...
      room: {
        ...
        room_city:data?.room_id?.room_city,
        room_slug: data?.room_id?.room_slug,
        room_location: data?.room_id?.room_location,
...
exports.getHostelBookingOrderByUserId = async (req, res) => {
  try {
    const myBooking = await HostelBookings.find({ booking_by: req.user.id })
      .populate('hostel_id')
      .populate('booking_by')
      .populate({
        path: 'reviews',
    ...
    // filtering booking orders based on different types query
    const bookingQuery = new MyQueryHelper(HostelBookings.find({ booking_by: req.user.id })
      .populate('hostel_id')
      .populate('booking_by')
      .populate(
        { path: 'reviews', populate: { path: 'user_id', model: 'Users' } }
    ...
    const mapperBooking = findBooking?.map((data) => ({
      ...
      reviews: !data?.reviews ? null : {
        id: data?.reviews.id,
        hostel_id: data?.reviews.hostel_id,
        booking_id: data?.reviews.booking_id,
        rating: data?.reviews.rating,
        message: data?.reviews.message,
        reviews_by: {
      ...
      room: {
        id: data?.hostel_id?._id,
        hostel_name: data?.hostel_id?.hostel_name,
        hostel_slug: data?.hostel_id?.hostel_slug,
        mess: data?.hostel_id?.mess,
        hostel_price: data?.hostel_id?.hostel_price,
        hostel_room: data?.hostel_id?.hostel_room,
        hostel_location: data?.hostel_id?.hostel_location,
        hostel_size: data?.hostel_id?.hostel_size,
        provide_breakfast: data?.hostel_id?.provide_breakfast,
        provide_lunch: data?.hostel_id?.provide_lunch,
        provide_dinner: data?.hostel_id?.provide_dinner,
        featured_hostel: data?.hostel_id?.featured_hostel,
        hostel_description: data?.hostel_id?.hostel_description,
        hostel_status: data?.hostel_id?.hostel_status,
        extra_facilities: data?.hostel_id?.extra_facilities,
        hostel_images: data?.hostel_id?.hostel_images?.map(
          (img) => ({ url: process.env.APP_BASE_URL + img.url })
        )
      },
      created_at: data?.createdAt,
...
exports.cancelSelfHostelBookingOrder = async (req, res) => {
  try {
    // finding by room id
    let booking = null;
    ...
    if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
      // find the booking by id and check if the booking_by user id matches the authenticated user id
      booking = await HostelBookings.findOne({ _id: req.params.id, booking_by: req.user.id });
    } else {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
...
exports.getHostelBookingOrderForAdmin = async (req, res) => {
  try {
    const myBooking = await HostelBookings.find()
      .populate('hostel_id')
      .populate('booking_by')
      .populate({
        path: 'reviews',
    ...
    // filtering booking orders based on different types query
    const bookingQuery = new MyQueryHelper(HostelBookings.find()
      .populate('hostel_id')
      .populate('booking_by')
      .populate(
        { path: 'reviews', populate: { path: 'user_id', model: 'Users' } }
...
exports.updatedHostelBookingOrderByAdmin = async (req, res) => {
  try {
    // finding by room by room id
    let booking = null;
    ...
    if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
      // find the booking by id and check if the booking_by user id matches the authenticated user id
      booking = await HostelBookings.findOne({ _id: req.params.id });
    } else {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
    ...
    if (/^[0-9a-fA-F]{24}$/.test(req.params.id)) {
      myRoom = await Hostels.findById(booking.hostel_id);
    } else {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
      ...
      case 'in-reviews':
        if (booking.booking_status === 'approved') {
          if (bookingDatesBeforeCurrentDate(booking?.booking_dates).isAnyDateInPast) {
            ...
            // update the room status to 'available'
            myRoom.hostel_status = 'available';
            await myRoom.save({ validateBeforeSave: false });
          } else {
            return res.status(400).json(errorResponse(
              1,
