...
const HostelBooking = require('../models/hostelbooking.model');
...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    const hosteltotalRooms = await Hostels.find();
    const hostelavailableRooms = await Hostels.find({ hostel_status: 'available' });
    const hostelunavailableRooms = await Hostels.find({ hostel_status: 'unavailable' });
    const hostelbookedRooms = await Hostels.find({ hostel_status: 'booked' });
    ...
    const hosteltotalBookings = await HostelBooking.find();
    const hostelpendingBookings = await HostelBooking.find({ booking_status: 'pending' });
    const hostelcancelBookings = await HostelBooking.find({ booking_status: 'cancel' });
    const hostelapprovedBookings = await HostelBooking.find({ booking_status: 'approved' });
    const hostelrejectedBookings = await HostelBooking.find({ booking_status: 'rejected' });
    const hostelinReviewsBookings = await HostelBooking.find({ booking_status: 'in-reviews' });
    const hostelcompletedBookings = await HostelBooking.find({ booking_status: 'completed' });
    ...
    res.status(200).json(successResponse(
      ...
      'Dashboard information get successful',
      {
        ...
        hostel_info: {
          total_rooms: hosteltotalRooms?.length || 0,
          available_rooms: hostelavailableRooms?.length || 0,
          unavailable_rooms: hostelunavailableRooms?.length || 0,
          booked_rooms: hostelbookedRooms?.length || 0
        },
        booking_info: {
        ...
        hostel_booking_info: {
          total_bookings: hosteltotalBookings?.length || 0,
          pending_bookings: hostelpendingBookings?.length || 0,
          cancel_bookings: hostelcancelBookings?.length || 0,
          approved_bookings: hostelapprovedBookings?.length || 0,
          rejected_bookings: hostelrejectedBookings?.length || 0,
          in_reviews_bookings: hostelinReviewsBookings?.length || 0,
          completed_bookings: hostelcompletedBookings?.length || 0
        }
      }
    ));
  } catch (error) {
